...
class Project:
  def __init__(self, entry):
    ...
    self.mc_versions = [version for version in p_info['mc_versions']]
    ...
    match self.type:
      case 'mod':
        for version in versions:
          if mc_version in version['mc_versions'] and loader_type in version['loaders'] and version['version_type'] == 'release':
            self.version = version
            return version
      case 'datapack':
        for version in versions:
          if mc_version in version['mc_versions'] and version['version_type'] == 'release':
            self.version = version
            return version
      case 'shaderpack' | 'resourcepack':
        for version in versions:
          if mc_version in version['mc_versions'] and 'iris' in version['loaders'] or 'minecraft' in version['loaders'] and version['version_type'] == 'release':
            self.version = version
            return version
        for version in versions:
      ...
      case _:
        ...
        return None
    logger.error('Correct version SOMEHOW could not be found!')
    return None
  ...
  def download(self, installation_folder: str):
    ...
    for file in self.version['files']:
      ...
      num_add = 0
      while Path(os.path.join(destination_path, file_name, f'({num_add})' if num_add != 0 else '')).exists():
        num_add += 1

      logger.info(f'Downloading {self.get_title()}, appr. {round(float(file['size'] / {1000000 if file['size'] > 1000000 else 1000}), 3)}{'MB' if file['size'] > 1000000 else 'KB'} in total')
